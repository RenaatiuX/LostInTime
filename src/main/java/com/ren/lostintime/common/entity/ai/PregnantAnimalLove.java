...
public class PregnantAnimalLove extends Behavior<Animal> {
    ...
    public PregnantAnimalLove(EntityType<? extends Animal> pPartnerType, float pSpeedModifier) {
    ...
    protected void start(ServerLevel pLevel, Animal pEntity, long pGameTime) {
        ...
        int i = MIN_DURATION + pEntity.getRandom().nextInt(MAX_DURATION - MIN_DURATION);
    ...
    protected void tick(ServerLevel pLevel, Animal pOwner, long pGameTime) {
        ...
        if (pOwner.closerThan(animal, BREED_RANGE)) {
            if (pGameTime >= this.spawnChildAtTime) {
                onBreeding(pLevel, pOwner, animal, pGameTime);
            }

        }
    }

    protected void onBreeding(ServerLevel level, Animal owner, Animal partner, long gameTime){
        owner.getBrain().setMemory(MemoryModuleType.IS_PREGNANT, Unit.INSTANCE);
